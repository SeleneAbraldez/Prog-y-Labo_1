...
void bajoPromedio(eJuegos* jueg, int lenJueg, float promedio){
    int bajoProm = 0;

    for(int j=0; j<lenJueg; j++){ //entendiendo que la fucnion aplica solo sobre la lista de juegos, y no sobre los juegos alquilados en su totalidad
        if(jueg[j].isEmpty == 0){
...
void listarClienteMasAlqui(eAlquiler* alqui, int lenAlqui, eJuegos* jueg, int lenJueg, eClientes* clien, int lenClien){
    ...
    for(int j=0; j<lenClien; j++){
        if(clien[j].isEmpty == 0){
            for(int i=0; i<lenAlqui; i++){
            if(alqui[i].isEmpty == 0){
                if(alqui[i].idJuego == clien[j].idC){
                    masCon++;
                    //printf("id %d - ", clien[j].idC);
                    //printf("%d ", masCon);
                }
            }
        }
