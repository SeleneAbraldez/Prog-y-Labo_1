...
{
    LinkedList* this;   //puntero
    this = (LinkedList *)malloc(sizeof(LinkedList));    //pido espacio din
    if(this != NULL){   //inicial
        this->size=0;
...
static Node* getNode(LinkedList* this, int nodeIndex)
{
    ...
    if(this != NULL && nodeIndex>=0 && nodeIndex<ll_len(this)){
        ...
        pNode = listAux.pFirstNode; //le asigno el primero por si es
        for(int i = 0; i<nodeIndex; i++){
           pNode = pNode->pNextNode; //le asigno al proximo buscando
        }
    }
...
int ll_add(LinkedList* this, void* pElement) //necesita addnode
{
    ...
    if(this !=NULL /*&& pElement!=NULL*/){
        if(addNode(this, ll_len(this), pElement) != -1){    //usa add node
            returnAux=0;
        }
    }
    return returnAux;
...
void* ll_get(LinkedList* this, int index)
{
    ...
    if(this!=NULL && index>= 0 && index<=ll_len(this)){
        gNode = getNode(this, index);   //usa get node de indez
        if(gNode!=NULL){
...
int ll_set(LinkedList* this, int index,void* pElement)
{
    ...
    if((this !=NULL) /*&& (pElement!=NULL)*/ && (index>=0) && (index<=(this->size)) ){
        gNode = getNode(this, index);   //get node con el index
        if(gNode!=NULL){
             gNode->pElement = pElement;    //asigno el valor de element a ese nodo
             returnAux=0;
        }
    }
...
int ll_push(LinkedList* this, int index, void* pElement)
{
    ...
    if((this!=NULL) /*&& (pElement!=NULL)*/ && (index>=0) && (index<=this->size) ){
        if(addNode(this, index, pElement)==0){  //agrega el nodo en el index
            returnAux = 0;
        }
    }
...
void* ll_pop(LinkedList* this,int index)
{
    void* returnAux = NULL;
    if((this!=NULL) && (index>=0) && (index<=this->size) ){
        returnAux = ll_get(this, index);    //consige el nodo
        if(returnAux != NULL){
            ll_remove(this, index);     //elimina si encuentra
        }
    }
...
LinkedList* ll_subList(LinkedList* this,int from,int to)
{
    ...
    if(this!=NULL){
        if((from>=0) && (from<ll_len(this)) && (to>from) && (to<=ll_len(this))){
            cloneArray = ll_newLinkedList();    //genera la nueva lista
            for(int i = from; i < to ; i++){    //espacio definido
                pElemAux = ll_get(this, i); //consigue el nodo
                ll_add(cloneArray, pElemAux);   //y agrega en nueva lista
            }
        }
    }

    return cloneArray;  //y devuelve puntero a clone
}
...
LinkedList* ll_clone(LinkedList* this)
{
    ...
    if(this!=NULL){
        cloneArray = ll_subList(this, 0, ll_len(this)); //clona con toda la lista
    }
    ...
    if(this!=NULL && pFunc!=NULL && (order == 0 || order == 1) ){
        for(int i=0;i<((ll_len(this))-1);i++){
            for(int j=i+1; j<(ll_len(this)); j++){
                if(order == 0 && pFunc(ll_get(this, i), ll_get(this, j))<0){    //orden 0
                    pElemAux = ll_get(this,i);
                    ll_set(this, i, ll_get(this,j));
                    ll_set(this, j, pElemAux);
                }else if(order == 1 && pFunc(ll_get(this, i), ll_get(this, j))>0){ //order 1
                    pElemAux = ll_get(this,i);
                    ll_set(this, i, ll_get(this,j));
                    ll_set(this, j, pElemAux);
                }
...
LinkedList* ll_filter(LinkedList* this, int(*pFunc)(void*)){
  LinkedList* filterList = ll_newLinkedList();      //creo lista filtrada
  if(this != NULL && pFunc != NULL){
    if(filterList != NULL){
      for(int i = 0; i<ll_len(this); i++){
        if(pFunc(ll_get(this, i))){     //si cumple retornando verdadero
          ll_add(filterList, ll_get(this, i));  //agrega a la lista filtrada
        }
      }
    }
  }
