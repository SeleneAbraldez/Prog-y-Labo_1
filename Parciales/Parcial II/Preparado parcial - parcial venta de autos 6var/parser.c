...
int parser_EmployeeFromText(FILE* pFile , LinkedList* pArrayListEmployee){
    ...
    cant = fscanf(pFile, "%[^;];%[^;];%[^;];%[^;];%[^;];%[^\n]\n", varId, varFecha, varCodigo, varCantidad, varPrecio, varCuit);                   // revisa la lectura y asigna a cantidad
    while(!feof(pFile)){                                                                                         //mientras no se llegue al final
        cant = fscanf(pFile, "%[^;];%[^;];%[^;];%[^;];%[^;];%[^\n]\n", varId, varFecha, varCodigo, varCantidad, varPrecio, varCuit);              // revisa la lectura y asigna a cantidad
        if(cant == 6){                                                                                          //si cantidad (la revision de lectura) es igual a 4
            pEmployee = employee_newParametros(varId, varFecha, varCodigo, varCantidad, varPrecio, varCuit);
