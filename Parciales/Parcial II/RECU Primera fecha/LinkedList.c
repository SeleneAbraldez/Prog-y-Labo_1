...
int ll_makeCreate(LinkedList* this)
{
  int retorno = -1;     //retorno
  ...
  int idAux, idCAux, importeAux, idAuxiolio=0;          //valores para agregar
...
///conteo
        ...
        if((empl->id) > idAuxiolio){  //se fija si es mayor a la ultima leida para no leer dos veces la misma
            conteoIdAbono++;
            abono = abono_new();
            idAuxiolio = empl->id;
            conteoId = 0;
