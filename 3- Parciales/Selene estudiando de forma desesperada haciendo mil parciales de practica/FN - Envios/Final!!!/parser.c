...
int parser_EntityFromText(FILE* pFile , LinkedList* pArrayListEntity){
    ...
    char varId[50], varNombre[50], varKm[50], varTipo[50], varCosto[50] = "0";
    int cant, i = 0;

    cant = fscanf(pFile, "%[^,],%[^,],%[^,],%[^\n]\n", varId, varNombre, varKm, varTipo);     // revisa la lectura y asigna a cantidad
    while(!feof(pFile)){                                                     //mientras no se llegue al final
        ...
        if(cant == 4){                                                                     //si cantidad (la revision de lectura) es igual a x cantidad de variables
            pEntity = entity_newParametros(varId, varNombre, varKm, varTipo, varCosto);
