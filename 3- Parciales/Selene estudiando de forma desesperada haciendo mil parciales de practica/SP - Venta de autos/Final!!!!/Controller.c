...
int controller_makeInfo(LinkedList* pArrayListEmployee)
{
    ...
    printf("\n*************************\n  Informe de ventas\n*************************\n");
    ventasTotales = ll_count(pArrayListEmployee, entityVentasTotales);
    if(ventasTotales == -1){
    ...
    mayoresA100000 = ll_count(pArrayListEmployee, entityMayores100000);
    if(mayoresA100000 == -1){
    ...
    mayoresA300000 = ll_count(pArrayListEmployee, entityMayores300000);
    if(mayoresA300000 == -1){
    ...
    ventasFord = ll_count(pArrayListEmployee, entityVentasFord);
    if(ventasFord == -1){
...
int controller_saveAsText(char* path , LinkedList* pArrayListEntity)
{
    ...
    fprintf(lista, "ID_venta,Fecha_Venta,Codigo_Producto,Cantidad,Precio_Unitario,Cuit_Cliente\n");       //primera linea
    if(pArrayListEntity!=NULL){
        for(int i=0; i<ll_len(pArrayListEntity); i++){
            this = (eEntity*)ll_get(pArrayListEntity, i);    //consigue entidad
            fprintf(lista, "%d,%s,%s,%d,%f,%s\n", this->id, this->fecha, this->codigo, this->cantidad, this->precio, this->cuit);   //y lo guarda
        }
