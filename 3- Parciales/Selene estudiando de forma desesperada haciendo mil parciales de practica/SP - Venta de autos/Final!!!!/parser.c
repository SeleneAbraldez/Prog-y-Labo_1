 ...
 *
 */
int parser_EntityFromText(FILE* pFile , LinkedList* pArrayListEntity){
    eEntity* pEntity;                                      //nueva entidad
    char varId[50], varFecha[50], varCodigo[50], varCantidad[50], varSueldo[50], varCuit[50];
    int cant, i = 0;

    cant = fscanf(pFile, "%[^;];%[^;];%[^;];%[^;];%[^;];%[^\n]\n", varId, varFecha, varCodigo, varCantidad, varSueldo, varCuit);     // revisa la lectura y asigna a cantidad
    while(!feof(pFile)){                                                     //mientras no se llegue al final
        cant = fscanf(pFile, "%[^;];%[^;];%[^;];%[^;];%[^;];%[^\n]\n", varId, varFecha, varCodigo, varCantidad, varSueldo, varCuit);      // revisa la lectura y asigna a cantidad
        if(cant == 6){                                                                     //si cantidad (la revision de lectura) es igual a x cantidad de variables
            pEntity = entity_newParametros(varId, varFecha, varCodigo, varCantidad, varSueldo, varCuit);
