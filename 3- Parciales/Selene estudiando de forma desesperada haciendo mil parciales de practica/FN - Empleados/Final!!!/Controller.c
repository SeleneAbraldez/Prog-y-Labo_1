...
int controller_menu(){    //menu inicial
    int option = -1;
        ...
        printf("2. Listar de manera asc por nombre \n");
...
int controller_ListEntitys(LinkedList* pArrayListEntity)
{
    ...
    if((ll_len(pArrayListEntity))>0){
        ...
        for(int i=0; i<ll_len(pArrayListEntity); i++){
            ...
            if(this != NULL){
                printf("%-10d %-26s %-26s %-10d %-19d  %-20.2f\n", this->id,this->nombre, this->empleo, this->edad, this->horasTrabajadas, this->sueldo);
            }
        }
    }else{
...
int controller_sortEntityMap(LinkedList* pArrayListEntity)
{
    int retorno = -1;
