...
int parser_EntityFromText(FILE* pFile , LinkedList* pArrayListEntity){
    ...
    char varId[50], varNombre[50], varEmpleo[50], varEdad[50], varHoras[50], varSueldo[50] = "0";
    int cant, i = 0;

    //cant = fscanf(pFile, "%[^,],%[^,],%[^,],%[^,],%[^\n]\n", varId, varNombre, varEmpleo, varEdad, varHoras);     // revisa la lectura y asigna a cantidad
    while(!feof(pFile)){                                                     //mientras no se llegue al final
        ...
        if(cant == 5){                                                                     //si cantidad (la revision de lectura) es igual a x cantidad de variables
            pEntity = entity_newParametros(varId, varNombre, varEmpleo, varEdad, varHoras, varSueldo);
