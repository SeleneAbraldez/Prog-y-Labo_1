 ...
 *
 */
int parser_EntityFromTextIngredientes(FILE* pFile , LinkedList* pArrayListEntity){
    eIngredientes* pEntity;                                      //nueva entidad
    char varId[50], varNombre[50], varCalorias[50], varGrasas[50], varSodio[50], varCarbohidratos[50], varProteinas[50];
    int cant, i = 0;

    cant = fscanf(pFile, "%[^,],%[^,],%[^,],%[^,],%[^,],%[^,],%[^\n]\n", varId, varNombre, varCalorias, varGrasas, varSodio, varCarbohidratos, varProteinas);     // revisa la lectura y asigna a cantidad
    while(!feof(pFile)){                                                     //mientras no se llegue al final
        cant = fscanf(pFile, "%[^,],%[^,],%[^,],%[^,],%[^,],%[^,],%[^\n]\n", varId, varNombre, varCalorias, varGrasas, varSodio, varCarbohidratos, varProteinas);      // revisa la lectura y asigna a cantidad
        if(cant == 7){                                                                     //si cantidad (la revision de lectura) es igual a x cantidad de variables
            pEntity = entity_newParametros(varId, varNombre, varCalorias, varGrasas, varSodio, varCarbohidratos, varProteinas);
...
int parser_EntityFromTextRecetas(FILE* pFile , LinkedList* pArrayListEntity){
    eIngredientes* pEntity;                                      //nueva entidad
    char varId[50], varNombre[50], varIdIngrediente[50], varCantidad[50];
    int cant, i = 0;

    cant = fscanf(pFile, "%[^,],%[^,],%[^,],%[^\n]\n", varId, varNombre, varIdIngrediente, varCantidad);     // revisa la lectura y asigna a cantidad
    while(!feof(pFile)){                                                     //mientras no se llegue al final
        cant = fscanf(pFile, "%[^,],%[^,],%[^,],%[^\n]\n", varId, varNombre, varIdIngrediente, varCantidad);      // revisa la lectura y asigna a cantidad
        if(cant == 4){                                                                     //si cantidad (la revision de lectura) es igual a x cantidad de variables
            pEntity = receta_newParametros(varId, varNombre, varIdIngrediente, varCantidad);
            ll_add(pArrayListEntity,pEntity);                                     //se agrega lo leido al array
            i++;                                                           //bytes leidos por si hay algun error
        }else{                                                  //hay algun error en la lectura
            if(feof(pFile)){                                             //si se llega al final del archivo
