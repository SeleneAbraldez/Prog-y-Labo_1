...
int ll_sumInt(LinkedList* this,int (*pFunc)(void*))
{
    int retorno=-1;
    int i;
    void* elemento;
...
float ll_sumFloat(LinkedList* this,float (*pFunc)(void*))
{
    float retorno = -1;
    int i;
    void* elemento;
...
LinkedList* ll_makeCreateComplete(LinkedList* this)
{
        ...
        if(abonoList != NULL) //se fija que se pudo reservar memoria
        {
            for(int i = 0; i<ll_len(this); i++)
            {
                ...
                if(cliente->id > idCliente)  //para no volver a analaizar uno que ya se analizo
                {
                    ...
                    for(int j = i+1; j<(ll_len(this)); j++)
                    {
                        conteoLL = ll_get(this, j);
                        clienteConteo = (eCliente*) conteoLL;
                        clienteConteoID = clienteConteo->id;
                    ...
                    itoa(idAutoincremental, idStr, 10);
                    itoa(tipo, tipoStr, 10);
                    itoa(idCliente, idClienteStr, 10);
                    itoa(importeFinal, importeFinalStr, 10);
