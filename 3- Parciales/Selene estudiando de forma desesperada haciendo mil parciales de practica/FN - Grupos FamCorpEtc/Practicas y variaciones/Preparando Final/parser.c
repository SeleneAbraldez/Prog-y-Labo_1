...
int parser_EmployeeFromText(FILE* pFile , LinkedList* pArrayListEmployee){
    Employee* pEmployee;
    char varId[50], varNombre[50], varSexo[50], varTelefono[50], varImporte[50];
    int cant, i = 0;

    cant = fscanf(pFile, "%[^,],%[^,],%[^,],%[^,],%[^\n]\n", varId, varNombre, varSexo, varTelefono, varImporte);                   // revisa la lectura y asigna a cantidad
    while(!feof(pFile)){                                                                                         //mientras no se llegue al final
        cant = fscanf(pFile, "%[^,],%[^,],%[^,],%[^,],%[^\n]\n", varId, varNombre, varSexo, varTelefono, varImporte);              // revisa la lectura y asigna a cantidad
        if(cant == 5){                                                                                          //si cantidad (la revision de lectura) es igual a 4
            pEmployee = employee_newParametros(varId, varNombre, varSexo, varTelefono, varImporte);
