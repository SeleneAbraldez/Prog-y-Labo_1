...
int controller_menu()     //menu inicial
{
    int option = -1;
    system("cls");
    printf("1. Cargar los datos de los libros (modo texto). \n");
    printf("2. Ordenar libros por editoriales asc\n");
    printf("3. Listar libros con editoriales \n");
    printf("4. Mapear librps y opcion de guardar lista\n");
...
int controller_ListEmployee(LinkedList* pArrayListEmployee)
{
    ...
    if((ll_len(pArrayListEmployee))>0)
    {
        ...
        for(int i=0; i<ll_len(pArrayListEmployee); i++)
        {
            ...
            auxEditorialId = this->editorialId;
            ...
            if(this != NULL)
            {
                printf("%-8d  %-50s  %-30s  %-8.1f  %-30s\n", this->id,this->titulo,this->autor,this->precio,editorialNombre);
            }
        }
    }
    else
...
//
//
int controller_sortPeliculasEditorialId(LinkedList* pArrayListEmployee)
{
    int retorno = -1;
    ll_sort(pArrayListEmployee, employeeSortByEditorialId, 0);
    retorno = 0;
    printf("Orden por editorialID asc realizado!\n");
    return retorno;
}
//
...
{
    int retorno = -1;
    LinkedList* mapedList;
    int optionYN;

    mapedList = ll_clone(pArrayListEmployee);

   if(ll_map(mapedList, employeeMapPrecios) != 0){
        printf("No se ha podido hacer el mapeo.");
    }else{
        printf("Precios actualizados con exito.\n");
        retorno = 0;
    }
    if(mapedList != NULL)
    {
        retorno = 0;
        if(ll_len(mapedList)>0)
        {
            controller_ListEmployee(mapedList);
            optionYN = getValidYesNoChar("Desea guardar la lista filtrada en 'mapeado.csv?' y/n: ", "Caracter no valido. ");
            if(optionYN == 'y')
            {
                controller_saveAsText("mapeado.csv", mapedList);
            }
            else
            {
                printf("Saliendo sin guardar. \n");
...
int controller_saveAsText(char* path, LinkedList* pArrayListEmployee)
{
    ...
    fprintf(lista, "id,titulo,autor,precio,editorialId\n");       //primera linea
    if(pArrayListEmployee!=NULL)
    {
        for(int i=0; i<ll_len(pArrayListEmployee); i++)
        {
            this = (Employee*)ll_get(pArrayListEmployee, i);    //consigue empelado
            fprintf(lista, "%d,%s,%s,%f,%d\n", this->id, this->titulo, this->autor, this->precio, this->editorialId);   //y lo guarda
        }
