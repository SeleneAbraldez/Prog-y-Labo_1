...
int parser_EmployeeFromText(FILE* pFile , LinkedList* pArrayListEmployee){
    ...
    char varId[50], varTitulo[50], varAutor[50], varPrecio[50], varEditorialId[50];
    int cant, i = 0;

    cant = fscanf(pFile, "%[^,],%[^,],%[^,],%[^,],%[^\n]\n", varId, varTitulo, varAutor, varPrecio, varEditorialId);     // revisa la lectura y asigna a cantidad
    while(!feof(pFile)){                                                     //mientras no se llegue al final
        cant = fscanf(pFile, "%[^,],%[^,],%[^,],%[^,],%[^\n]\n", varId, varTitulo, varAutor, varPrecio, varEditorialId);      // revisa la lectura y asigna a cantidad
        if(cant == 5){                                                                     //si cantidad (la revision de lectura) es igual a 4
            pEmployee = employee_newParametros(varId, varTitulo, varAutor, varPrecio, varEditorialId);
