...
int parser_EmployeeFromText(FILE* pFile , LinkedList* pArrayListEmployee){
    ...
    char varId[50], varNombre[50], varAnio[50], varGenero[50];
    int cant, i = 0;

    //cant = fscanf(pFile, "%[^,],%[^,],%[^,],%[^\n]\n", varId, varNombre, varAnio, varGenero);     //primera linea
    while(!feof(pFile)){                                                     //mientras no se llegue al final
        cant = fscanf(pFile, "%[^,],%[^,],%[^,],%[^\n]\n", varId, varNombre, varAnio, varGenero);      // revisa la lectura y asigna a cantidad
        if(cant == 4){                                                                     //si cantidad (la revision de lectura) es igual a 4
            pEmployee = employee_newParametros(varId, varNombre, varAnio, varGenero);
