...
int controller_menu(){    //menu inicial
    int option = -1;
        ...
        printf("4. Depurar peliculas, listarlas y opcion de guardar depurado\n");
        printf("5. Filtrar peliculas por genero y opcion de guardar filtrado\n");
        printf("10. Salir \n");
...
int controller_filterEmployee(LinkedList* pArrayListEmployee)
{
    ...
    if(filterList != NULL){
        retorno = 0;
        if(ll_len(filterList)>0){
            controller_ListPeliculas(filterList);
...
int controller_saveAsText(char* path , LinkedList* pArrayListEmployee)
{
    ...
    fprintf(lista, "id,nombre,año,genero\n");       //primera linea
    if(pArrayListEmployee!=NULL){
        for(int i=0; i<ll_len(pArrayListEmployee); i++){
...
void controller_listOne(LinkedList* pArrayListPeliculas, int index)
{
    ...
    employee_getId(auxPelicula, &auxId);
    employee_getNombre(auxPelicula,auxNombre);
    employee_getAnio(auxPelicula,&auxAnio);
    employee_getGenero(auxPelicula,auxGenero);
